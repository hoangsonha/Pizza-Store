...
<html>
    <head>
        ...
        function chooseFile(fileInput) {
            ...
            // tạo ra FileList và lưu vào trong files
            // files chỉ tạo ra khi đặt hàm choosèile trong thẻ input từ type='file'
            //  
            //fileInput.files[0].type, fileInput.files[0].size
            
            // b1 : kiểm tra xem có phải file hay k và kiểm tra phần tử
            //      đầu tiên của file xem có trống k
            // b2 : tạo ra đối tg Filereader và gắn sự kiện onload()
            // b3 : cập nhật nguồn hình ảnh, e.target.result chính là 
            //      đoạn url + base64 của file thêm vào att src trong thẻ img
            ...
            if(fileInput.files && fileInput.files[0]) {
                ...
                // dòng này là đọc file dưới dạng base64
            }
        }
...
<aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link collapsed" data-bs-target="#components-nav" data-bs-toggle="collapse" href="MainController?action=Product_view">
      ...
      <li class="nav-item">
        <a class="nav-link collapsed" data-bs-target="#forms-nav" data-bs-toggle="collapse" href="MainController?action=Account_view">
          <i class="bi bi-journal-text"></i><span>Account</span><i class="bi bi-chevron-down ms-auto"></i>
        </a>
      </li><!-- End Forms Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" data-bs-target="#tables-nav" data-bs-toggle="collapse" href="HomeController">
            ...
            <div class="form-holder">
                <div class="form-content">
                    <div class="form-items">
                        ...
                        <form class="requires-validation" action="MainController" method="post">

                            <div class="col-md-12">
                                <input class="form-control" type="text" name="productID" value="${requestScope.PRODUCT.productID}" readonly="">
                            </div>
                            <div style="color: white; margin: 10px 15px 0 15px; font-size: 16px">${requestScope.ERROR.getErrorID()}</div>
                            <div class="col-md-12">
                               <input class="form-control" type="text" name="productName" value="${requestScope.PRODUCT.productName}" required>
                            </div>
                            <div style="color: white; margin: 5px 15px 0 15px; font-size: 16px">${requestScope.ERROR.getErrorName()}</div>
                            <div class="col-md-12">
                                <select class="form-select mt-3"  style="margin-top: 0px" name="categoryID" required>
                                    <option value="${requestScope.PRODUCT.categoryID}">Category : ${requestScope.PRODUCT.categoryID}</option>
                            ...
                            <div class="col-md-12" style="margin-top: 15px;">
                               <input class="form-control" type="number" name="quantity" value="${requestScope.PRODUCT.quantityPerUnit}" required min="1">
                           </div>
                            <div class="col-md-12" style="margin-top: 15px;">
                               <input class="form-control" type="number" name="price" value="${requestScope.PRODUCT.unitPrice}" required min="1">
                           </div>
                           <div class="col-md-12" style="margin-top: 15px;">
                               <img src="image/${requestScope.PRODUCT.productImg}" id="fileImage" width="200" height="200">                          
                               <input onchange="chooseFile(this)" title=" " class="form-control" type="file" id="productImg" name="productImg">
                           </div>
                            <div class="form-button mt-3">
                                <button style="margin-top: 10px; padding: 10px" id="submit" type="submit" class="btn btn-primary" name="action" value="Update">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
//    var text = document.getElementById('im').value;
//        
//        console.log(text);
